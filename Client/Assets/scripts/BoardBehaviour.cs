...
public class BoardBehaviour : MonoBehaviour
{
    ...
    internal static void BattleResult(object[] data)
    {
        ...
        endTimeNode = Math.Max(abattle.Count, bbattle.Count) * GameConfig.BATTLE_CARD_INTERVAL;
        ...
        for (int i = 0; i < amods.Count; i ++)
        {
            ...
            float time = endTimeNode + (GameConfig.BATTLE_CARD_INTERVAL +
                ...
                GameConfig.BATTLE_CARD_EFFECT_LABEL_PAUSE) * i;
            // pop out active card
            s.Insert(time, item.transform.DOMove(new Vector3(item.TempPos.x, item.TempPos.y, item.TempPos.z - 1f), GameConfig.BATTLE_CARD_EFFECT_SCALE_TIME));
            ...
            s.Insert(time +
                ...
                GameConfig.BATTLE_CARD_EFFECT_HIGHLIGHT_PAUSE * 2.0f +
                GameConfig.BATTLE_CARD_EFFECT_LABEL_SCALE_TIME +
                GameConfig.BATTLE_CARD_EFFECT_LABEL_PAUSE, item.transform.DOMove(item.TempPos, GameConfig.BATTLE_CARD_EFFECT_SCALE_TIME));
            s.Insert(time +
                GameConfig.BATTLE_CARD_EFFECT_SCALE_TIME +
                GameConfig.BATTLE_CARD_EFFECT_HIGHLIGHT_PAUSE * 2.0f +
                GameConfig.BATTLE_CARD_EFFECT_LABEL_SCALE_TIME +
        ...
        endTimeNode += (GameConfig.BATTLE_CARD_INTERVAL +
            ...
            GameConfig.BATTLE_CARD_EFFECT_HIGHLIGHT_PAUSE * 2.0f +
            GameConfig.BATTLE_CARD_EFFECT_LABEL_SCALE_TIME +
            GameConfig.BATTLE_CARD_EFFECT_LABEL_PAUSE) * Math.Max(amods.Count, dmods.Count);
        ...
        s.InsertCallback(endTimeNode, () =>
        {
            if (aname.Equals(LocalPlayer.PlayerName))
            {
                ...
                {
                    LocalPlayer.CSob[acs].MoveToGrave(s, endTimeNode + GameConfig.BATTLE_CARD_INTERVAL * i, CardObjectBehaviour.GetCOB(akilled.ElementAt(i).guid).gameObject);
                }
                for (int i = 0; i < dkilled.Count; i++)
                {
                    EnemyPlayer.CSob[dcs].MoveToGrave(s, endTimeNode + GameConfig.BATTLE_CARD_INTERVAL * i, CardObjectBehaviour.GetCOB(dkilled.ElementAt(i).guid).gameObject);
                }
            }
            else if (dname.Equals(LocalPlayer.PlayerName))
            {
                ...
                for (int i = 0; i < akilled.Count; i++)
                {
                    EnemyPlayer.CSob[acs].MoveToGrave(s, endTimeNode + GameConfig.BATTLE_CARD_INTERVAL * i, CardObjectBehaviour.GetCOB(akilled.ElementAt(i).guid).gameObject);
                }
                for (int i = 0; i < dkilled.Count; i++)
                {
                    LocalPlayer.CSob[dcs].MoveToGrave(s, endTimeNode + GameConfig.BATTLE_CARD_INTERVAL * i, CardObjectBehaviour.GetCOB(dkilled.ElementAt(i).guid).gameObject);
                }
            }
